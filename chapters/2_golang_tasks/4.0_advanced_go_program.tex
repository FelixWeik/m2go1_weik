...
The class \texttt{CarRentalCLI} implements the cli-commands by calling the according functions from the \texttt{CarRentalOperationsInterface}.
This provides great overview and abstraction, improving the maintainability and scalability of the application.

\paragraph*{Logic:}
This package is responsibile for managing the state of the application and for performing the business logic.
...
Both components share the same model, which is defines in the \texttt{model} subpackage.
